//! `revoq doctor` — environment and toolchain diagnostics.
//!
//! These checks are deliberately kept OUT of `revoq build`'s hot path: a build
...
//! (`revoq doctor`) or when a build has just failed, at which point a slow,
...
fn print_report(checks: &[Check], verbose: bool) {
    println!("\x1b[1;36mrevoq doctor\x1b[0m — environment diagnostics\n");
    ...
    if failed == 0 {
        println!(
            "\x1b[1;32mAll {} checks passed.\x1b[0m Your environment is ready for `revoq build`.",
            checks.len()
        );
    } else {
        println!(
            "\x1b[1;33m{passed} passed, {failed} failed.\x1b[0m Fix the items above, then re-run `revoq doctor`."
        );
    }
    if verbose {
...
/// If the current directory has a `revoq.toml` with a `[package] toolchain`
...
fn check_toolchain_pin() -> Option<Check> {
    ...
    Some(match spec.validate() {
        ...
        Err(e) => Check {
            ...
            detail: e.to_string(),
            fix: Some(format!(
                "install/select {} {} (or update [package] toolchain in revoq.toml)",
    ...
    Check {
        ...
        fix: Some(
            "install curl (e.g. `sudo apt install curl`, `brew install curl`) — \
             required for `revoq sync` and dependency reachability probing"
                .into(),
        ),
    }
}
...
fn check_system_headers() -> Check {
    ...
    let probe_src = dir.join(format!("revoq-doctor-{}.c", std::process::id()));
...
fn check_revoq_home() -> Check {
    let home = match std::env::var("REVOQ_HOME") {
        Ok(h) if !h.is_empty() => PathBuf::from(h),
        _ => match std::env::var("HOME") {
            Ok(h) if !h.is_empty() => PathBuf::from(h).join(".revoq"),
            _ => {
                return Check {
                    name: "revoq home",
                    ok: false,
                    detail: "neither $REVOQ_HOME nor $HOME is set".to_string(),
                    fix: Some("export HOME (or REVOQ_HOME) so revoq can locate its cache".into()),
                };
            }
        },
    };
...
mod tests {
    ...
    /// `doctor` stays project-agnostic by default: with no `revoq.toml`
    ...
    fn toolchain_check_is_absent_without_a_pinned_manifest() {
        // This crate's own repo root has no revoq.toml, so running the test
