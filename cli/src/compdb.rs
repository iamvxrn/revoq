...
//!
//! Every successful `revoq build` writes a clangd-compatible compilation
//! database to the project root, so IDEs and language servers (VS Code,
//! Neovim, CLion, ...) pick up revoq's exact standard version, warnings,
...
pub struct CompileCommandEntry {
    ...
    /// from — revoq always runs clang with its own inherited cwd, so this is
    ...
    /// (`clang`/`clang++` first, then every flag revoq generated for this
...
/// there. Pretty-printed since, unlike revoq's `--json` payloads, this file
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("revoq-compdb-test-{label}-{}", std::process::id()));
