...
//! `target_link_libraries` — and emits a starter `revoq.toml`. CMake's
//! scripting constructs (`foreach`, `function`, `if`, generator expressions,
//! ...) are not interpreted; when present, revoq just flags them so the user
//! knows to port that logic by hand. No CMake-parsing crate is pulled in —
//! this is plain string scanning, in keeping with revoq's "no heavy deps"
//! rule.
...
///
/// `revoq migrate` must never abort on a mixed-language project — CMake places
/// no single-language restriction on a target, but revoq does. Rather than
...
impl CMakeProject {
    fn render_manifest(&self) -> String {
        ...
        out.push_str("# Generated by `revoq migrate --from=cmake`.\n");
        out.push_str(
            "# Review the TODOs below — revoq's CMake parser only understands a few\n\
        ...
        if !self.conflicting_sources.is_empty() {
            ...
            out.push_str(&format!(
                "# revoq enforces one language per package; {dominant} was chosen as the\n\
    ...
    fn print_notices(&self) {
        if !self.sources.is_empty() {
            ...
            println!(
                "\x1b[1;33mnote\x1b[0m: revoq uses a strict layout — there is no \"sources\" \
                 list in revoq.toml. Move/merge these detected sources into {entry} by hand:"
            );
        ...
        if !self.complex_hits.is_empty() {
            println!(
                "\x1b[1;33mnote\x1b[0m: CMakeLists.txt uses constructs revoq does not parse \
    ...
    /// conflict with revoq's single-language rule. This is the migration
...
fn parse_cmake(raw: &str) -> CMakeProject {
    ...
    for call in extract_calls(&text, "target_include_directories") {
        let mut tokens = call.split_whitespace();
        tokens.next(); // target name — revoq has one implicit target per package
        for tok in tokens {
    ...
    // a mix — CMake permits mixed-language targets even though revoq doesn't.
    // Ties favor C++, matching `revoq init`'s own default.
