...
//! `target_link_libraries` — and emits a starter `revol.toml`. CMake's
//! scripting constructs (`foreach`, `function`, `if`, generator expressions,
//! ...) are not interpreted; when present, revol just flags them so the user
//! knows to port that logic by hand. No CMake-parsing crate is pulled in —
//! this is plain string scanning, in keeping with revol's "no heavy deps"
//! rule.
...
///
/// `revol migrate` must never abort on a mixed-language project — CMake places
/// no single-language restriction on a target, but revol does. Rather than
...
impl CMakeProject {
    fn render_manifest(&self) -> String {
        ...
        out.push_str("# Generated by `revol migrate --from=cmake`.\n");
        out.push_str(
            "# Review the TODOs below — revol's CMake parser only understands a few\n\
        ...
        if !self.conflicting_sources.is_empty() {
            ...
            out.push_str(&format!(
                "# revol enforces one language per package; {dominant} was chosen as the\n\
    ...
    fn print_notices(&self) {
        if !self.sources.is_empty() {
            ...
            println!(
                "\x1b[1;33mnote\x1b[0m: revol uses a strict layout — there is no \"sources\" \
                 list in revol.toml. Move/merge these detected sources into {entry} by hand:"
            );
        ...
        if !self.complex_hits.is_empty() {
            println!(
                "\x1b[1;33mnote\x1b[0m: CMakeLists.txt uses constructs revol does not parse \
    ...
    /// conflict with revol's single-language rule. This is the migration
...
fn parse_cmake(raw: &str) -> CMakeProject {
    ...
    for call in extract_calls(&text, "target_include_directories") {
        let mut tokens = call.split_whitespace();
        tokens.next(); // target name — revol has one implicit target per package
        for tok in tokens {
    ...
    // a mix — CMake permits mixed-language targets even though revol doesn't.
    // Ties favor C++, matching `revol init`'s own default.
