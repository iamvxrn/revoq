...
//! surface of the `revoq` binary. No logic lives here beyond what clap needs to
//! parse arguments. The runtime engine consumes these structures in `main.rs`.

use clap::{Parser, Subcommand};
use std::path::PathBuf;

/// revoq — a modern package manager and build system for C and C++.
#[derive(Parser, Debug)]
#[command(
    name = "revoq",
    version,
    about = "A modern package manager and build system for C and C++.",
    long_about = "revoq is a build system for C and C++ with strict \
...
/// All top-level subcommands revoq understands.
...
pub enum Command {
    ...
    /// Create a new revoq package in the given directory (or current dir).
    Init(InitArgs),

    /// Re-resolve project dependencies and rewrite revoq.lock.
    ///
    /// Touches only the current project's dependency graph: cache checkouts
    /// under `~/.revoq/cache` and `revoq.lock`. Never touches the package index
    /// (`~/.revoq/revoq-libs`) — see `Sync` for that.
    Update(UpdateArgs),

    /// Diagnose the local toolchain and environment (clang, ar, headers, ...).
    Doctor,

    /// Refresh the local package index (~/.revoq/revoq-libs) from the registry.
    ///
    /// Touches only that one flat-text index file via native OS fetch tools.
    /// Never resolves dependencies, never touches a project's `revoq.lock` —
    /// see `Update` for that.
    Sync,

    /// Generate a starter revoq.toml from an existing build system's config.
    Migrate(MigrateArgs),

    /// Vendor every dependency in revoq.lock into a local third_party/ tree.
    ///
    /// Once third_party/ is populated, subsequent `revoq build` runs resolve
...
pub struct BuildArgs {
    ...
    /// result into `target/<profile>/revoq_profile.json` (loadable at
...
/// Arguments for `revoq check`.
///
/// Deliberately a smaller surface than `BuildArgs`: `revoq check` never
...
mod tests {
    ...
    fn sync_and_update_resolve_to_distinct_variants() {
        let sync = Cli::try_parse_from(["revoq", "sync"]).expect("sync should parse");
        match sync.command {
            Command::Sync => {}
            other => panic!("expected Command::Sync, got {other:?}"),
        }

        let update = Cli::try_parse_from(["revoq", "update"]).expect("update should parse");
        ...
        assert!(Cli::try_parse_from(["revoq", "sync", "update"]).is_err());
    }

    #[test]
    fn update_accepts_an_optional_package_argument() {
        let cli = Cli::try_parse_from(["revoq", "update", "mylib"]).unwrap();
        match cli.command {
            Command::Update(args) => assert_eq!(args.package, Some("mylib".to_string())),
            other => panic!("expected Command::Update, got {other:?}"),
        }

        let cli = Cli::try_parse_from(["revoq", "update"]).unwrap();
        match cli.command {
    ...
    fn sync_takes_no_positional_arguments() {
        assert!(Cli::try_parse_from(["revoq", "sync", "extra"]).is_err());
    }
    ...
    fn json_flag_is_global_and_defaults_to_false() {
        let cli = Cli::try_parse_from(["revoq", "build"]).unwrap();
        assert!(!cli.json);

        let before = Cli::try_parse_from(["revoq", "--json", "build"]).unwrap();
        assert!(before.json);

        let after = Cli::try_parse_from(["revoq", "doctor", "--json"]).unwrap();
        assert!(after.json);
    }
    ...
    fn build_target_flag_defaults_to_none_and_parses_when_given() {
        ...
        let cli = Cli::try_parse_from(["revoq", "build", "--target", "aarch64-unknown-linux-gnu"])
            .unwrap();
        match cli.command {
            Command::Build(args) => {
    ...
    fn build_from_and_ignore_warnings_default_off_and_parse_when_given() {
        ...
        let cli =
            Cli::try_parse_from(["revoq", "build", "--from", "legacy/src", "--ignore-warnings"])
                .unwrap();
        match cli.command {
            Command::Build(args) => {
                assert_eq!(
    ...
    /// `revoq run` flattens `BuildArgs`, so the legacy build flags must be
    /// reachable through it too.
    #[test]
    fn run_inherits_from_and_ignore_warnings_flags() {
        let cli =
            Cli::try_parse_from(["revoq", "run", "--from", "app", "--ignore-warnings"]).unwrap();
        match cli.command {
            Command::Run(args) => {
                assert_eq!(
    ...
    fn check_parses_as_its_own_command_with_build_like_flags() {
        let cli = Cli::try_parse_from(["revoq", "check"]).unwrap();
        match cli.command {
            Command::Check(args) => {
