//! Manifest (`revoq.toml`) and lockfile (`revoq.lock`) data model.
//!
...
/// The fully parsed `revoq.toml` manifest.
...
pub struct Package {
    ...
    /// Optional pinned toolchain, e.g. `"clang-18.1"`. When set, `revoq doctor`
    /// and the pre-build phase of `revoq build` invoke the active compiler and
    ...
    /// during both compilation and linking. `revoq build --target <triple>`
    ...
    /// `revoq build --from <path>` overrides this at the CLI (see
    ...
    /// for building noisy legacy code you don't own. `revoq build
    ...
    /// revoq no longer requires a canonically-named `main.*`/`lib.*` entry
...
impl Manifest {
    /// Load and parse a `revoq.toml` from a directory root.
    pub fn load(root: &Path) -> Result<Manifest> {
        let path = root.join("revoq.toml");
        let text = fs::read_to_string(&path).path_ctx(&path)?;
        let manifest: Manifest = toml::from_str(&text).map_err(|e| RevoqError::ManifestParse {
            path: path.clone(),
            message: e.to_string(),
        })?;
        Ok(manifest)
...
/// The complete `revoq.lock` file: a flat list of locked dependencies.
#[derive(Debug, Clone, Deserialize, Serialize, Default)]
pub struct Lockfile {
...
impl Lockfile {
    /// Load a `revoq.lock` if it exists. Returns `Ok(None)` when absent.
    pub fn load(root: &Path) -> Result<Option<Lockfile>> {
        let path = root.join("revoq.lock");
        if !path.exists() {
            return Ok(None);
        }
        let text = fs::read_to_string(&path).path_ctx(&path)?;
        let lock: Lockfile = toml::from_str(&text).map_err(|e| RevoqError::LockParse {
            path: path.clone(),
            message: e.to_string(),
        })?;
        Ok(Some(lock))
    ...
    pub fn save(&self, root: &Path) -> Result<()> {
        ...
        let header = "# This file is auto-generated by revoq.\n\
                      # It records exact resolved versions for reproducible builds.\n\
                      # Do not edit by hand; run `revoq update` to regenerate.\n\n";
        let body =
            toml::to_string_pretty(&sorted).map_err(|e| RevoqError::Serialize(e.to_string()))?;
