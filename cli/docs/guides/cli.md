...
Complete command and flag reference for the `revoq` binary, derived from the
`clap` definitions in [cli.rs](../src/cli.rs) and the dispatch logic in
[main.rs](../src/main.rs).

```
revoq [-v|--verbose]... [-q|--quiet] <COMMAND>
```
...
- **`-v` / `--verbose` (repeatable counting flag).** Uses
  ...
  same single verbose mode (extra `[engine]`/`[resolver]`/`[revoq]` diagnostic
...
- **`-q` / `--quiet`.** A plain boolean. Declared with
  ...
  `-q` with `-v`/`--verbose` at the argument-parsing stage, before revoq's own
  ...
  `quiet`) or the unconditional `revoq migrate` unmapped-source warning (see
...
- **`--json`.** Also `global = true`, so it parses before or after any
  subcommand. Only `revoq build` and `revoq doctor` currently act on it; other
  ...
  revoq's three-dependency footprint unchanged (see
...
```
revoq build [--release] [-o NAME] [-j N] [--manifest-path DIR]
...
| `-j`, `--jobs N` | `Option<usize>`. Resolved by `resolve_jobs()` in [main.rs](../src/main.rs) (shared with `revoq check`): `explicit.unwrap_or_else(default_jobs).max(1)`. This is the **clamping**: an explicit `-j` is floored to a minimum of `1` (so `-j 0` cannot spawn zero workers), and an absent `-j` falls back to `std::thread::available_parallelism()`. `Engine::new` applies a second floor (`jobs.max(1)`) and `compile_all`/`check_package` further clamp the *actual* worker count to `self.jobs.min(total)` — never more threads spawned than there are translation units to process. |
| `--manifest-path DIR` | `Option<PathBuf>`. May point at a directory or directly at a `revoq.toml` file (`project_root` strips the filename in the latter case). Defaults to the current working directory. Resolution fails fast with `LayoutViolation` if no `revoq.toml` is found at the resolved root. |
...
| `--from PATH` | `Option<PathBuf>` (legacy support). The directory revoq scans for sources and the entry point, overriding `[package] source_dir` for this invocation. `effective_source_dir` ([main.rs](../src/main.rs)) resolves **`--from` > `source_dir` > `"src"`**. Inherited by `revoq run`. See [manifest.md](manifest.md#legacy-support--source_dir-include_dirs-defines-ignore_warnings). |
| `--ignore-warnings` | Boolean (legacy support). Injects `-w` to silence every compiler warning, regardless of the `[package] ignore_warnings` field — either source turns it on. `-w` lands after the profile's `-W` groups (so it wins) but before `extra_flags`. Never applied to `revoq check`. |
...
those local copies plus `revoq.lock` metadata — no `git`, no network, no
global resolver cache lookups (`vendored_dependencies` in
[main.rs](../src/main.rs)). See [`revoq vendor`](#revoq-vendor) below for how
...
`revoq.toml`) and constructs one `Compiler` for the whole package — the same
...
exists at `~/.revoq/cache/prebuilt/{key}/lib{name}.a` (`.lib` on Windows), the
...
new toolchain-management machinery in revoq itself.
...
shared by every compile path, including `revoq check`'s analysis pass —
...
cache](#revoq-build) above) always differs from a native build of the same
library — the two can never collide in `~/.revoq/cache/prebuilt/`.

**No triple validation.** revoq does not maintain or check against a list of
...
- **Entries are planned before the cache-hit check.** `build_package` builds
  ...
  `~/.revoq/cache/prebuilt/{hash}` (see [Global build
  cache](#revoq-build) above) still contributes accurate
...
- **One entry per translation unit**, matching the schema field-for-field:
  `directory` (the revoq process's own `cwd`, via `std::env::current_dir()` —
  every clang invocation inherits it, since revoq never calls
...
- Written by `cmd_build` after `build_single`/`build_workspace` returns
  ...
  `RevoqError`, same as any other artifact write in revoq.

### Build profiling (`--trace`)

`--trace` turns on Clang's [`-ftime-trace`](https://clang.llvm.org/docs/UsersManual.html#profiling-clang)
frontend/backend profiler and has revoq make sense of its output. Two pieces,
both new in v0.5.0:
...
1. **Flag injection** (`compiler.rs`). `Compiler::new` takes a `trace: bool`
   ...
   `~/.revoq/cache/prebuilt/`, and a stale cache hit can never silently
...
2. **Aggregation and reporting** (`trace.rs`, invoked from
   ...
   extension — revoq relies on that convention (via `-o`) rather than passing
   ...
   - Writes the merged document to
     `target/<debug|release>/revoq_profile.json` — standard [Chrome Trace
...
every `revoq build` flag documented above is also a valid `revoq run` flag with
identical semantics — `revoq run` is implemented as "build, then exec" with no
...
- **Validation against library crates.** After `build_with_diagnostics`
  ...
  `RevoqError::LayoutViolation("\`revoq run\` requires an executable
...
- **Verbatim argument forwarding.** `#[arg(last = true)]` is clap's "greedy
  ...
  revoq flags, not split/escaped/re-quoted. These are passed straight through
  to the child process via `Command::new(&outcome.artifact).args(&args.bin_args)`.
  This is why `revoq run --release -- --release` correctly applies `--release`
  to the *build* once and forwards the literal string `--release` as the
  binary's own argv — clap stops parsing revoq's own flags at the first bare
  `--`.
- The child's exit status is propagated: a non-zero exit causes
  `std::process::exit(status.code().unwrap_or(1))` from the revoq process
  itself, so shell scripts checking `$?` after `revoq run` see the *binary's*
  exit code, not revoq's.

### `revoq init`

```
revoq init [PATH] [--name NAME] [--lib | --bin] [--c]
```
...
- **Language/kind selection.** `--lib` and `--bin` are mutually exclusive
  ...
  |---|---|---|---|
  | no | no | `src/main.cpp` | `CPP_MAIN` (`#include <iostream>`, prints "Hello from revoq!") |
  | no | yes | `src/main.c` | `C_MAIN` (`#include <stdio.h>`, `printf`) |
  | yes | no | `src/lib.cpp` | `CPP_LIB` (a `revoq_add(int, int)` function) |
  | yes | yes | `src/lib.c` | `C_LIB` (same, C-flavored comment style) |

- **Overwrite protection.** Before writing, `cmd_init` checks
  `entry_path.exists()` and returns `RevoqError::LayoutViolation("... already
  exists; refusing to overwrite")` — init never clobbers an existing entry
  file. The manifest (`revoq.toml`) and `.gitignore` get the same treatment but
  via simple existence checks that silently skip writing rather than erroring
  (`if !manifest_path.exists() { ... }`), so re-running `revoq init` in an
...
6. **The end-to-end compilation probe.** `check_system_headers` writes a
   ...
   (`revoq-doctor-<pid>.c`), containing exactly:
   ```c
   #include <stdio.h>
...
7. `$REVOQ_HOME` (or `$HOME/.revoq` if unset) is locatable. This check always
   reports `ok: true` even when the directory doesn't exist yet — it only
   fails hard if *neither* `$REVOQ_HOME` nor `$HOME` is set at all, since the
...
`doctor` is invoked two ways: explicitly via `revoq doctor`, and automatically
(non-fatally — `let _ = doctor::run(verbose);`) by `build_with_diagnostics`
whenever a `revoq build` or `revoq run` invocation fails, right before revoq
...
Refreshes the **flat-text package index** at `~/.revoq/revoq-libs` (or
`$REVOQ_HOME/revoq-libs`) — the shorthand-to-URL mapping table used to resolve
...
or writes a project's `revoq.lock`. The doc comments in both
[cli.rs](../src/cli.rs) and [resolver.rs](../src/resolver.rs) call this out
explicitly to distinguish it from `revoq update`.
...
overridable via the `REVOQ_LIBS_URL` environment variable (for self-hosted or
...
- **Unix**: `fetch_with_curl_or_wget` tries
  ...
  surface a `RevoqError`.

**Atomicity.** The fetch writes to a sibling `revoq-libs.tmp` file first, then
...
since the rename is the only operation that touches the real `revoq-libs`
path.

### `revoq update`

```
revoq update [PACKAGE] [--manifest-path DIR]
```

Re-resolves the **current project's** dependency graph from scratch and
rewrites `revoq.lock` — the inverse operation to `revoq sync` (which never
touches `revoq.lock`) and complementary to `revoq build` (which, by design,
*reads* the lock and never silently re-resolves on its own — see
[manifest.md](manifest.md#revoqlock-spec)).
...
- **Scoped update** (`PACKAGE` given): the existing lock *is* passed as `pin`
  ...
  → `lib`), so `revoq update lib` matches regardless of which shorthand prefix
  was used.
- **Dependency cache state.** Re-resolution does not necessarily mean
  ...
  `~/.revoq/cache/<name>-<tag>` if it already contains a `.git` directory,
...
- The rewritten lockfile is written via the same atomic `.tmp` + `rename`
  pattern as `revoq sync`'s index (see [manifest.md](manifest.md#revoqlock-spec)).
- Non-quiet output prints one `Updated N dependenc{y,ies} in revoq.lock` line
...
```
revoq vendor [--manifest-path DIR]
```

Copies every dependency recorded in `revoq.lock` into a local
`<root>/third_party/<name>/` tree, for complete offline autonomy — once that
directory is populated, every subsequent `revoq build` resolves dependencies
from it directly (see [Offline/vendored dependency
resolution](#revoq-build) above), with no `git`, no network, and no global
`~/.revoq/cache` lookups at all.

`cmd_vendor` ([main.rs](../src/main.rs)):

1. Requires an existing `revoq.lock` (`revoq build` or `revoq update` must have
   run at least once) — refuses with `RevoqError::Config` otherwise, rather
   than silently re-resolving.
2. Resolves dependencies via `Resolver::resolve_all(&manifest, Some(&lock))`
   — the same *pinned* path `revoq build` takes, so vendoring never drifts
...
3. For each resolved dependency, recursively copies its global-cache
   ...
   `third_party/<name>/` is removed first, so re-running `revoq vendor` is
   idempotent.
...
```
revoq check [--manifest-path DIR] [-j N] [--features A,B,C]
...
dependencies exactly like `revoq build` does — respecting a populated
`third_party/` the same way (see [Offline/vendored dependency
resolution](#revoq-build) above) — but only to expose their `src/`/`include/`
directories on the include path via `-I`. Dependencies are never compiled,
analyzed, or even type-checked by `revoq check`; it audits the package you're
...
reused verbatim for both `revoq build` and `revoq check`, never touches
...
- **Only a non-zero clang exit counts as a failure.** Analyzer findings
  ...
  contract `revoq build` already has. A file clang couldn't even parse
  (genuine syntax error, missing header, ...) does fail the command.

A failure returns `RevoqError::Analysis { failures }` — a variant distinct
from `RevoqError::Compilation` ([error.rs](../src/error.rs)) purely so the
top-line message reads `check failed: N file(s) could not be analyzed`
rather than the misleading `build failed: ...`, since `revoq check` never
...
terminal as it arrived, and (unlike `revoq build --json`) `revoq check` has no
...
**`revoq build --json`.** `cmd_build_top_level` ([main.rs](../src/main.rs))
...
recompiled — see the [Global build cache](#revoq-build) section above.
`errors` carries the structured `CompileDiagnostic`s attached to
`RevoqError::Compilation` when the failure came from the compiler; for any
...
**`revoq doctor --json`.** `doctor::run` ([doctor.rs](../src/doctor.rs)) runs
the exact same checks as the human-readable report (see [`revoq
doctor`](#revoq-doctor) above, including the conditional `toolchain` check)
and renders:
