...
This document describes how `revoq` is put together internally: the design
...
`revoq` is a build system and package manager for C and C++. Three commitments
shape every module:

**A manifest-driven mindset for C/C++.** The user-facing shape — `init`,
`build`, `run`, a `revoq.toml` manifest, a `revoq.lock` lockfile, `[features]`,
...
`revoq.toml` declares package metadata, features, and compiler profiles;
`revoq.lock` pins every dependency to an exact commit. Each package is a
...
Every place revoq would normally reach for a library, it instead shells out to
...
- `--json` output, `compile_commands.json`, and reading Clang's
  ...
  scoped to the JSON documents revoq actually needs to read back (Chrome
  Trace Event Format) — it is not a general-purpose JSON library.

This keeps the revoq binary itself small and fast to build, and keeps revoq's
own supply chain trivially auditable.

**Strict layout enforcement.** revoq does not glob for source files and does
...
If none exists, the build fails immediately with `RevoqError::LayoutViolation`
...
`revoq build`'s defining performance goal is a near-instant invocation when the
...
1. Locate `revoq.toml` (`project_root`) — one `is_file` check.
2. Parse the manifest (`Manifest::load`) — one `read_to_string` + TOML parse.
3. Discover the layout (`Layout::assert_revoq_standard`) — directory/file
...
— lives exclusively in `revoq doctor` ([doctor.rs](../src/doctor.rs)), which a
healthy `revoq build` never runs.
...
fn build_with_diagnostics(args: BuildArgs, verbose: bool, quiet: bool) -> Result<BuildOutcome> {
    match cmd_build(args, verbose, quiet) {
        ...
        Err(err) => {
            // only on failure: print a note, then run `revoq doctor::run`
            ...
        }
    }
}
...
*already failed* does revoq pay for the comparatively expensive, exhaustive
...
function returns `RevoqError::Compilation { failures }` if any unit failed,
...
hard `RevoqError::LayoutViolation` rather than silently compiled. A revoq
...
Producing a static library is the one step in revoq where "the same tool
...
- **Windows**: two candidates, most-preferred first —
  1. `llvm-ar rcsD <archive> <objects...>` (LLVM's `ar`-compatible archiver,
     present if the user has LLVM installed — likely, since revoq already
...
let output = match Command::new(&link.program).args(&link.args).output() {
    ...
    Err(source) => {
        ...
        return Err(RevoqError::CommandSpawn { ... }); // last candidate also missing
    }
};
```
...
reported immediately as `RevoqError::CommandFailed` with clang-diagnostic-style
formatting where applicable, and revoq does **not** silently fall through to
...
Two v0.5.0 additions — `compile_commands.json` and `revoq build --trace` —
follow the same "derive it from what revoq already computed, don't add a new
...
`directory` (the revoq process's own cwd), `file` (the unit's source path),
...
database is exactly what revoq would run, because it's built from the same
...
[cli.md](cli.md#revoq-build)) calls those same two functions, a `--trace`
...
`--target` and `revoq check` (v0.5.0) both extend existing machinery rather
...
**`revoq check` reuses the build engine's spawn-and-parse path instead of
...
analyzer findings without revoq treating them as a distinct kind of
diagnostic.
...
never reaches, so `revoq check` never constructs them in the first place
